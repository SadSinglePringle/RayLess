...
$args = @(
    ...
    "/Isrc\astg",
    "/Isrc\diagnostics",
    "src\diagnostics\m2_challenger1_harness.cpp",
    "/link",
    "/LIBPATH:$MSVC_LIB",
    "/LIBPATH:$WIN_SDK_LIB",
    "/LIBPATH:$WIN_SDK_UCRT",
    "/LIBPATH:bin",
    "/OUT:bin\m2_challenger1_harness.exe",
    "astg_rtx.lib",
    "d3d12.lib",
    "dxgi.lib",
    "d3dcompiler.lib"
...
Write-Host "Compiling bin\m2_challenger1_harness.exe with MSVC..."
& $cl $args

if (Test-Path "bin\m2_challenger1_harness.exe") {
    Write-Host "✅ Successfully built bin\m2_challenger1_harness.exe!" -ForegroundColor Green
} else {
    Write-Error "❌ Build failed for m2_challenger1_harness.exe"
}

...
func trace_and_deposit_diffuse_bounces(
	...
	lights: Array[Light3D],
	probes: Array[SurfaceProbe],
	out_bounce1_nodes: Array[TransportNode]
) -> int:
	...
	for node in bounce0_nodes:
		...
		var reflected_radiance = light_energy * node_weight * node.albedo
		...
		# Trace coarse hemisphere distribution (Bounce 1)
		for s in range(diffuse_samples_per_node):
			var r1 = randf()
			var r2 = randf()
			...
			if hit_res.hit:
				...
				var bounce_factor = geom_term * (0.25 / (PI * float(diffuse_samples_per_node)))
				var bounce_radiance = reflected_radiance * bounce_factor

				# Create Bounce 1 Transport Node
				var b1_node = TransportNode.new()
				b1_node.id = out_bounce1_nodes.size()
				b1_node.source_id = node.source_id
				b1_node.parent_node_id = node.id
				b1_node.bounce_depth = 1
				b1_node.hit_position = hit2_pos
				b1_node.hit_normal = hit2_norm
				b1_node.incoming_direction = world_dir
				b1_node.destruction_chunk_id = hit_res.chunk_id
				b1_node.surface_cluster_id = _find_closest_cluster(hit2_pos, hit2_norm)
				b1_node.indirect_radiance = bounce_radiance
				b1_node.state = node.state
				out_bounce1_nodes.append(b1_node)
				node.child_node_ids.append(b1_node.id)

				# Record collapsed source->bounce1->probe transfer in Contribution Table
				if contribution_table != null:
					...
					_record_transfer_to_probes(hit2_pos, hit2_norm, b1_node.surface_cluster_id, node.source_id, diffuse_transfer, probes)

				# Surface-aware deposition and TERMINATE branch into probe field
				var dep = SurfaceAwareProbeLookup.deposit_to_surface_probes(
					hit2_pos,
					hit2_norm,
					b1_node.surface_cluster_id,
					bounce_radiance,
					clusters,
					probes,
					false # is_indirect
